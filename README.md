...
**Data:** Offshore LIDAR and buoy meteorological data from Morro Bay, California (DOE A2E dataset). 10 minute average data for all variables are used. A few days worth of sample wind speed data and meterological data is available inside the *data_ LIDAR* and *data_BUOY* folders respectively for testing.  
...
raw_data_preprocessing.py  →  wind_data_cleaned.csv 
                               met_data_merged.csv (subtitutes missing data point time syncs all sensor data)
        │
        ▼
VMD.py                     →  vmd_series/vmd_results_wspd{height}.npz (Performs VMD decomposition)
        │
        ▼
datagen.py                 →  Train / Val / Test PyTorch Datasets
        │
        ▼
hyperparam_optuna.py       →  best_params_{model}.json
        │
        ▼
train_final.py             →  best_{model}.pt
```
